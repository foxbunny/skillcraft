...
behavior, the history that led here, and the likely trajectory**. The boundary: it **explains; it
does not review.** No verdicts, no severity-ranked findings, no "fix before merge," and no code
changes. This is the "explain" slot — distinct from a review (judgement) or a verify (does it run).
...
**Top guardrail: explain, don't judge — and skip the plumbing, out loud.** Spend the words on the
...
- **Build real context, don't guess.** Understand the system the code plugs into well enough to
  explain the *why*: fan out a read-only explore over conventions/callers, read the neighbouring and
  canonical code, and read any relevant design doc. Cite concrete anchors (`file:line`, commit SHAs,
  PR/ticket numbers) so the walkthrough is verifiable.

### 2. Write the walkthrough
Adapt these sections to the mode; drop any that don't apply; lead with the punchline.
...
3. **The substantive changes / parts** — grouped logically (not file-by-file), each with a focused
   ...
   **explicitly skip** the plumbing, saying you're skipping it.
...
5. **History leading up to this** — the lineage: prior tickets/PRs/commits that built the surrounding
   machinery, and what this stacks on. From the log + ticket context, not invention.
6. **Likely trajectory** — where this is heading: the obvious next asks, the follow-on it sets up,
   known rough edges someone will revisit. Grounded, not generic.
...
- Where design docs / architecture notes live, and the project's "plumbing" categories worth skipping
...
- **Honesty over optimism** — cite real anchors and ground the "why" in the ticket and code; flag a
...
- Fills the explain slot alongside judgement (`/code-review`, [adversarial-skeptic](adversarial-skeptic.md))
  and verification (a verify command). No built-in command covers in-depth explanation, so this is a
  genuinely additive archetype.
